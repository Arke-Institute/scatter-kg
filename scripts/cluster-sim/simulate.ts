 ...
 *   --mutual               Only cluster with peers who have you in their top-K (mutual relationship)
 */
...
interface SimConfig {
  ...
  seed: number;
  verbose: boolean;
  mutual: boolean;  // Require mutual top-K for clustering
}
...
class ClusterSimulation {
  ...
  /**
   * Check if the relationship between two entities is mutual (both in each other's top-K)
   */
  private isMutualPeer(entityId: string, peerId: string): boolean {
    if (!this.config.mutual) return true; // mutual mode off, always true

    // Check if entityId is in peerId's top-K
    const peerSims = this.similarities[peerId] || [];
    const peerTopK = peerSims.slice(0, this.config.k);
    return peerTopK.some(s => s.peerId === entityId);
  }

  private findClusteredPeer(entityId: string, peers: SimilarityEntry[]): { peerId: string; clusterId: string } | null {
    for (const peer of peers) {
      const peerState = this.entityStates.get(peer.peerId);
      if (peerState?.clusterId && this.isMutualPeer(entityId, peer.peerId)) {
    ...
    switch (event.type) {
      ...
      case 'entity_searches': {
        ...
        if (peers.length === 0) {
          ...
          const clusteredPeer = this.findClusteredPeer(event.entityId, peers);
  ...
  private runFallback(entityId: string, myClusterId: string, layer: number) {
    ...
    for (const peer of visibleSemanticPeers) {
      const peerState = this.entityStates.get(peer.peerId);
      if (peerState?.clusterId && peerState.clusterId !== myClusterId && this.isMutualPeer(entityId, peer.peerId)) {
